...
int main(void)
{
    ...
    struct Celula *hashTable[25]; // hash table destruct Celulas;
    for (int c = 0; c < 25; c++)
    {
    ...
    while (testaFim(caminhoArquivo) == false)
    {
        ...
        struct Celula *celula = novaCelula(personagem);
        contadorComparacao++;
        if (hashTable[personagem.altura % 25]->atual == NULL)
        {

            hashTable[personagem.altura % 25]->atual = celula;
        }
        else
        {
            struct Celula *tmp = celula;                           // inicializa o personagem recebido na célula tmp
            tmp->prox = hashTable[personagem.altura % 25]->prox; // pega a referência de próximo da primeira célula da lista
            hashTable[personagem.altura % 25]->prox = tmp;         // atualiza a referência da primeira célula com tmp
        }
        scanf(" %[^\n]s", caminhoArquivo);
        getchar();
    }
    ...
    while (testaFim(stringRecebida) == false)
    {
        ...
        char caminho[] = strcat(caminho, stringRecebida);
        ...
        else if (strcmp(hashTable[personagem.altura % 25]->atual->nome, personagem.nome) == 0)
        {
            contadorComparacao++;
            printf("SIM\n");
        }
        else
        {
            contadorComparacao++;
            int resp = 0;
            struct Celula *tmp = &hashTable[personagem.altura % 25]; // recebe a posição 0 da lista
            while (resp == 0 && tmp->prox != NULL)
            {
                ...
                tmp = tmp->prox;
                if (strcmp(hashTable[personagem.altura % 25]->atual->nome, personagem.nome) == 0)
                {
                    contadorComparacao++;
