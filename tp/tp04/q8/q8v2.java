...
class ArvoreTrie {
    ...
    public boolean pesquisar(String nome, No no, int posicao) throws Exception {
        boolean resp;
        if (no.prox[nome.charAt(posicao)] == null) {
            resp = false;
        } else if (posicao == nome.length() - 1) {
            resp = (no.prox[nome.charAt(posicao)].folha == true);
        } else if (posicao < nome.length() - 1) {
            resp = pesquisar(nome, no.prox[nome.charAt(posicao)], posicao + 1);
        } else {
    ...
    private void inserir(Personagem personagem, No no, int posicao) throws Exception {

        if (no.prox[personagem.getNome().charAt(posicao)] == null) {
            no.prox[personagem.getNome().charAt(posicao)] = new No(personagem.getNome().charAt(posicao));

            numeroComparacoes++;

            if (posicao == personagem.getNome().length() - 1) {
                numeroComparacoes++;

                no.prox[personagem.getNome().charAt(posicao)].folha = true;
            } else {
                inserir(personagem, no.prox[personagem.getNome().charAt(posicao)], posicao + 1);
            }

        } else if (no.prox[personagem.getNome().charAt(posicao)].folha == false && posicao < personagem.getNome().length() - 1) {
            numeroComparacoes++;
