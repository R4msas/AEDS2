...
void ordenaPorInsercao(Personagem listaPersonagem[], int tamanhoVetor, int contador[])
{
    for (int i = 1; i < tamanhoVetor; i += 1)
    {
        Personagem personagemTemporario = listaPersonagem[i];
        int j = i - 1;
        while ((j >= 0) && ((listaPersonagem[j].altura > personagemTemporario.altura) || ((listaPersonagem[j].altura == personagemTemporario.altura) && (strcmp(listaPersonagem[j].nome, personagemTemporario.nome) > 0))))
        {
...
void criarLog(time_t inicio, int contador[])
{
    ...
    FILE *log = fopen("790152_quicksort.txt", "w");
...
void ordenaQuickSort(Personagem listaPersonagem[], int contador [],int esq, int dir)
{
int i=esq, j =dir;
Personagem pivo= listaPersonagem[i+j/2];
while(i<=j)
{
    ...
    {
        contador[1]+=3;
        swapPersonagem(listaPersonagem,i,j);
        i++;
        j--;
    }
}
if(esq<j)
{
    ordenaQuickSort(listaPersonagem,contador, esq, j);
}
if(i<dir)
{
    ordenaQuickSort(listaPersonagem,contador, i, dir);
...
printf("teste leitura");
    ...
    printf("teste término leitura");


    ordenaQuickSort(listaPersonagem, contador,0, contadorTamanho-1); // Ordena pela cor do cabelo por quicksort
    ordenaPorInsercao(listaPersonagem, contadorTamanho, contador); // Ordena pelo nome por Inserção no caso do cabelo ser igual
