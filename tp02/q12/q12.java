...
public class q12 {
    ...
    public static String ordenaShellSort(Personagem[] arrayDePersonagens, int posicaoFinalDoArray)
    {
        int contadorPosicao = 0;
        int contadorComparacao = 0; 
        long inicioTempo = System.nanoTime();
        int intervalo=posicaoFinalDoArray/2;
        while(intervalo>0)
        {
            for (int c=intervalo;c<posicaoFinalDoArray;c++)
            {
               for(int i=c;i>=intervalo;i--)
               {
               contadorComparacao++;
               MyIO.println(arrayDePersonagens[i].nome+" ,peso do primeiro: "+arrayDePersonagens[i].peso+"\n"+arrayDePersonagens[c].nome+" peso"+arrayDePersonagens[c].peso);
               if(arrayDePersonagens[i].peso<arrayDePersonagens[c].peso)
               {
               swapPersonagem(arrayDePersonagens,i,c);
               MyIO.println("troca a posição. Nova ordem:"+arrayDePersonagens[i].nome+"//"+arrayDePersonagens[c].nome);
               contadorPosicao++;
                }
            }
        }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        Arq.openWriteClose(nomeDoArquivo, ordenaShellSort(arrayDePersonagens, posicaoArray));
